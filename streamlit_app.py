...
H5_PAGE_TITLE = "XMODhub单机游戏修改器开发 | 支持你喜欢游戏的修改器优先上线！"
...
def build_activity_page() -> str:
    ...
    favicon_uri = image_data_uri(FAVICON_PATH) if FAVICON_PATH.exists() else ""
    if favicon_uri:
        html = re.sub(r"<title>.*?</title>", f"<title>{H5_PAGE_TITLE}</title>", html, flags=re.IGNORECASE | re.DOTALL)
        html = re.sub(r'<link\s+rel="icon"[^>]*>\s*', "", html, flags=re.IGNORECASE)
        html = html.replace("</head>", f'  <link rel="icon" type="image/png" href="{favicon_uri}" />\n</head>')
    else:
        html = re.sub(r"<title>.*?</title>", f"<title>{H5_PAGE_TITLE}</title>", html, flags=re.IGNORECASE | re.DOTALL)
...
def main() -> None:
    ...
    st.set_page_config(
        page_title="AI 免费评估记录后台" if admin_view else H5_PAGE_TITLE,
        page_icon=str(FAVICON_PATH) if FAVICON_PATH.exists() else "X",
        layout="wide",
