...
NO_DEMO_TECH_DETAIL = "经评估，该游戏暂时无法被赞助。当前未查询到该游戏或其 Demo 的 Unity 引擎信息，因此暂不支持直接参与赞助。建议您前往 XMODhub客户端为该游戏投票，后续运营团队会根据投票热度和技术可行性再次评估。"
...
def save_game_query(game: dict, xmod_status: dict, decision: dict) -> None:
    ...
    with db_connect() as conn:
        ...
        existing_failure_detail = existing["failure_detail"] if existing else ""
        failure_detail = existing_failure_detail or (text_value(decision.get("detail")) if not decision.get("passed") else "")
...
def manual_overrides_for_app(appid: str) -> dict:
    ...
    return {
        ...
        "failure_detail": text_value(row["failure_detail"]),
    }
...
def evaluate_steamdb_game(game: dict, xmod_status: dict | None = None, demo_game: dict | None = None) -> dict:
    ...
    if not contains_token(categories, "Single-player"):
        return {"passed": False, "title": "该游戏暂不支持赞助", "detail": SPONSORSHIP_REJECT_DETAIL, "basis": [f"categories 不包含 Single-player：{categories or '-'}"]}
    for blocked in ["MMO", "In-App Purchases", "Adult Only"]:
        if contains_token(categories, blocked):
            return {"passed": False, "title": "该游戏暂不支持赞助", "detail": SPONSORSHIP_REJECT_DETAIL, "basis": ["categories 包含 Single-player", f"categories 包含 {blocked}"]}
...
def render_streamlit_activity_page() -> None:
    ...
    with right:
        if not result_payload:
            ...
            if decision["passed"]:
                record_id = result_payload.get("record_id", "")
                tracking_url = f"?pay_click={quote(record_id)}&redirect=1"
                st.markdown(
                    f"""
                    <div class="pay-action-wrap">
                      <a class="pay-link" href="{html.escape(tracking_url)}" target="_blank" rel="noopener">
                        前往支付
                      </a>
                    </div>
                    """,
...
def render_streamlit_admin_page() -> None:
    ...
    st.markdown(
        ...
        <style>
          ...
          .block-container { max-width: 1500px; padding: 48px 28px 64px; }
