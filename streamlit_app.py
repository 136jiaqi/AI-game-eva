...
def save_game_query(game: dict, xmod_status: dict, decision: dict) -> None:
    ...
    now = now_text()
    with db_connect() as conn:
        existing = conn.execute("SELECT manual_result, note, query_count FROM game_queries WHERE appid = ?", (appid,)).fetchone()
        ...
        conn.execute(
            ...
            INSERT INTO game_queries (
              ...
              client_status, client_development_status, evaluation_result, auto_passed, manual_result, note, basis, query_count, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        if not appid:
            ...
            with st.spinner("正在评估，请稍候..."):
                try:
                    ...
                    final_passed = final_passed_for_app(appid, bool(decision["passed"]))
...
def render_streamlit_admin_page() -> None:
    ...
    with tab_assets:
        ...
        edited = st.data_editor(
            ...
            disabled=[col for col in (rows[0].keys() if rows else []) if col not in {"人工结果", "备注"}],
            key="asset_editor",
        )
        if st.button("保存人工结果/备注"):
            init_db()
            with db_connect() as conn:
                for row in editor_records(edited):
                    conn.execute(
                        "UPDATE game_queries SET manual_result = ?, note = ?, updated_at = ? WHERE appid = ?",
                        (text_value(row.get("人工结果")) or "自动", text_value(row.get("备注")), now_text(), text_value(row.get("APP ID"))),
                    )
            st.success("已保存")
